...
public class UndeadTeam extends EntityData{
	...
	public Entity getTarget(EntityNecromancerMinion minion){
		...
		if(master != null && position.getSquaredDistance(new Position(master)) < 48){
			...
			if(tt.equals(TargetType.DEFENSIVE)){
				...
				return EntityUtils.getNearestEntityInList(position, worldObj.selectEntitiesWithinAABB(EntityLiving.class, AxisAlignedBB.getBoundingBox(minion.posX - d, minion.posY - d, minion.posZ - d, minion.posX + d, minion.posY + d, minion.posZ + d), IMob.mobSelector));
			}
			else if(tt.equals(TargetType.OFFENSIVE)){
				List list = worldObj.getEntitiesWithinAABBExcludingEntity(minion, AxisAlignedBB.getBoundingBox(minion.posX - d, minion.posY - d, minion.posZ - d, minion.posX + d, minion.posY + d, minion.posZ + d));
