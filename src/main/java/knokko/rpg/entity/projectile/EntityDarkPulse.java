...
public class EntityDarkPulse extends EntityThrowable{
	...
	public void onImpact(MovingObjectPosition mop) {
		if(mop.entityHit instanceof EntityLivingBase && mop.entityHit != thrower){
			...
			EntityDamageSourceIndirect source = new EntityDamageSourceIndirect("dark pulse", this, thrower);
	...
	public void onUpdate(){
		super.onUpdate();
		if(thrower == null && throwerId != null && !throwerId.isEmpty()){
			thrower = (EntityLivingBase) EntityUtils.getEntityByUUID(worldObj, throwerId);
		}
		if(thrower == null){
			thrower = getThrower();
		}
	...
	public void writeEntityToNBT(NBTTagCompound nbt){
		if(thrower != null){
			nbt.setString("throwerId", thrower.getUniqueID().toString());
		}
	...
	public void readEntityFromNBT(NBTTagCompound nbt){
		...
		throwerId = nbt.getString("throwerId");
