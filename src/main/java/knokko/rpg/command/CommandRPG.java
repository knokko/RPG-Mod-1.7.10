...
public class CommandRPG implements ICommand{
	public static final ChatComponentTranslation useAge = new ChatComponentTranslation(EnumChatFormatting.RED + "You must use rpg mana/class/help/race/xp/team/special/spell/options/stats");
			...
			if(string.length > 0){
				if(string[0].matches("mana")){
					if(string.length == 1){
						...
						if(player.canCommandSenderUseCommand(0, "effect")){
					...
					if(string.length > 1){
						if(string[1].matches(WorldData.getString(player, "class", "hunter"))){
							player.addChatMessage(new ChatComponentTranslation("You are allready " + string[1]));
						}
					...
					if(string.length == 2){
						...
						if(!WorldData.isIntegerPositive(player, "special cooldown")){
							if(special.matches("rage")){
								...
								int y = 0;
								int times = 0;
								while(times < 100 && (y < 1 || player.worldObj.isAnyLiquid(AxisAlignedBB.getBoundingBox(x-1, y-1, z-1, x+1, y+1, z+1)))){
									y = BlockUtils.getHighestBlock(player.worldObj, (int)x, (int) z);
									++times;
								}
								if(y > 0 && !player.worldObj.isAnyLiquid(AxisAlignedBB.getBoundingBox(x-1, y-1, z-1, x+1, y+1, z+1))){
									player.setPositionAndUpdate(x, y + 1, z);
									WorldData.setInteger(player, 600, "special cooldown");
								}
							}
							else if(special.matches("iron bash")){
								List entities = player.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, AxisAlignedBB.getBoundingBox(player.posX - 15, player.posY - 15, player.posZ - 15, player.posX + 15, player.posY + 15, player.posZ + 15));
								int times = 0;
						...
						if(string[1].matches("help")){
								...
								if(WorldData.isLeader(team, player)){
									...
									if(player2 != null){
										if(!WorldData.getBooleanOption(player2, WorldData.invite + team, false)){
											...
											player.addChatMessage(new ChatComponentTranslation(EnumChatFormatting.GREEN + "You have invited " + player2.getCommandSenderName() + " to your team."));
										}
										else {
							...
							if(string.length == 2){
								...
								if(player2 != null){
									if(WorldData.isOnSameTeam(player, player2)){
										...
										player.addChatMessage(new ChatComponentTranslation(EnumChatFormatting.YELLOW + "You have kicked " + player2.getCommandSenderName() + " from your team."));
										player2.addChatMessage(new ChatComponentTranslation(EnumChatFormatting.RED + player.getCommandSenderName() + " has kicked you from you team."));
									}
									else {
							...
							if(string.length == 2){
								...
								if(WorldData.isLeader(team, player)){
									...
									if(player2 != null){
										if(!WorldData.isLeader(team, player2)){
											player.addChatMessage(new ChatComponentTranslation(EnumChatFormatting.GREEN + "You have succesfully promoted " + player2.getCommandSenderName() + " to leader."));
											player2.addChatMessage(new ChatComponentTranslation(EnumChatFormatting.GREEN + player.getCommandSenderName() + " has promoted you to a leader."));
										}
										else {
