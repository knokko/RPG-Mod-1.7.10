			...
			if(Class.matches("mage")){
				if(name.matches("ice")){
					...
					if(mana >= 500 * manaUse){
						...
						List entities2 = world.getEntitiesWithinAABBExcludingEntity(player, AxisAlignedBB.getBoundingBox(pos.x - spirit, pos.y - spirit, pos.z - spirit, pos.x + spirit, pos.y + spirit, pos.z + spirit));
						times = 0;
						while(times < entities2.size()){
							Entity entity = (Entity) entities2.get(times);
							double distance = pos.getSquaredDistance(new Position(entity));
							if(distance <= spirit){
								if(distance < 2){
									distance = 2;
								}
					...
					line.spawnParticles(world, 1.1, 1, 0.1, ExtraUtils.divineAccurate(0.1, line.distance), 5);
				...
				if(name.matches("lightshield")){
					...
					if(mana >= 50000 * manaUse){
						player.addPotionEffect(new PotionEffect(11, 1200, 3));
						WorldData.removeFromInteger(player, "mana", ExtraUtils.fromDouble(50000 * manaUse), 0);
						WorldData.setInteger(player, 50, "cooldown");
						EntityUtils.sendManaMessage(player);
					}
				}
				...
				if(name.matches("heal") && mana >= 5000 * manaUse){
					if(!world.isRemote){
						...
						while(times < healPower * 0.4){
							...
							world.spawnParticle("heart", player.posX - 0.5 + random.nextDouble(), player.posY - 0.5 + random.nextDouble(), player.posZ - 0.5 + random.nextDouble(), 0, 0, 0);
							++times;
						}
					}
				}
				...
				if(name.matches("ice")){
						...
						if(mana >= 5000 * manaUse && partner.shouldHeal()){
							...
							if(!world.isRemote){
								...
								while(times2 < healPower * 0.4){
									Random random = new Random();
									world.spawnParticle("heart", partner.posX - 0.5 + random.nextDouble(), partner.posY - 0.5 + random.nextDouble(), partner.posZ - 0.5 + random.nextDouble(), 0, 0, 0);
									++times2;
								}
							}
						}
					...
					if(entity != null && mana >= 5000 * manaUse && entity.getHealth() < entity.getAttributeMap().getAttributeInstance(SharedMonsterAttributes.maxHealth).getBaseValue()){
						if(!world.isRemote){
							...
							while(times < healPower * 0.4){
								Random random = new Random();
								world.spawnParticle("heart", entity.posX - 0.5 + random.nextDouble(), entity.posY + 0.5 + random.nextDouble(), entity.posZ - 0.5 + random.nextDouble(), 0, 0, 0);
								++times;
							}
						}
					}
				...
				if(name.matches("fire")){
					...
					if(WorldData.getInteger(player, "mana") >= 10000 * manaUse){
						...
						List entities2 = world.getEntitiesWithinAABBExcludingEntity(player, AxisAlignedBB.getBoundingBox(target.x - spirit, target.y - spirit, target.z - spirit, target.x + spirit, target.y + spirit, target.z + spirit));
						int times = 0;
						while(times < entities.size()){
							Entity entity = (Entity) entities.get(times);
						...
						while(times < entities2.size()){
							Entity entity = (Entity) entities2.get(times);
							double distance = target.getSquaredDistance(new Position(entity));
							if(distance <= spirit){
								if(distance < 2){
									distance = 2;
								}
				...
				else if(name.matches("undeadcontrol") && !world.isRemote && mana > 10000 * manaUse){
						...
						while(times < entities.size()){
							...
							if(living != entity && living.getCreatureAttribute() == EnumCreatureAttribute.UNDEAD){
								...
								if(living instanceof EntityLiving){
									((EntityLiving)living).setAttackTarget(entity);
								}
							}
							++times;
						}
				...
				else if(name.matches("pigmanstrike") && !world.isRemote && mana >= 5000 * manaUse){
					Line line = Line.fromRaytrace(player, 100).toNearestBlock(world, false, 1);
					EntityLivingBase entity = (EntityLivingBase) line.getNearestEntity(world, EntityLivingBase.class, line.getPosition(1), player);
					if(entity != null){
						int times = 0;
						while(times < RPG.getPlayerSpirit(player)){
							EntityPigZombie pig = new EntityPigZombie(world);
							pig.setPosition(player.posX, player.posY, player.posZ);
							pig.setTarget(entity);
							pig.setCurrentItemOrArmor(0, new ItemStack(Items.golden_sword));
