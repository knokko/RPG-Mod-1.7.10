...
public class BuildUtils {
	...
	public static void placeBlockInCircle(World world, int x, int y, int z, Block block, Position base, double range, double buffer){
		...
		if(position.getSquaredDistance(base) >= range - buffer && position.getSquaredDistance(base) <= range + buffer){
			world.setBlock(x, y, z, block);
		}
	}
	...
	public static void makeVulcano(World world, Block block, Random random, int x, int y, int z){
		...
		int lenght = height;
		world.setBlock(x, y + height + 1, z, Blocks.lava);
		int x2 = x - lenght;
		while(x2 <= x + lenght){
			int z2 = z - lenght;
			while(z2 <= z + lenght){
				try {
					int height2 = (int) ((new Position(x2, height, z2).getSquaredDistance(new Position(x, height, z))) * 2);
