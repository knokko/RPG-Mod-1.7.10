...
public class Line {
	...
	public void spawnParticles(World world, double red, double green, double blue, double dbp, int lifetime){
		...
		while(times <= particles && world.isRemote){
			EntityReddustFX dust = new EntityReddustFX(world, x, y, z, (float) ExtraUtils.divineAccurate(1, lifetime), 1, 1);
	...
	public static Line fromRaytrace(Entity entity, double lenght){
		...
		Vec3 v3 = Vec3.createVectorHelper(pos1.x, pos1.y, pos1.z);
		Position pos4 = new Position(v3.addVector(pos2.x * lenght, pos2.y * lenght, pos2.z * lenght));
		Position pos3 = new Position(entity.posX * pos2.x, (entity.height * 0.8 + entity.boundingBox.minY) * pos2.y, entity.posZ * pos2.z);
	...
	public List getBlocks(World world, boolean acceptWeakBlocks){
		...
		while (times <= maxTimes){
			...
			if(acceptWeakBlocks || block.getBlockHardness(world, (int) x, (int) y, (int) z) != 0){
